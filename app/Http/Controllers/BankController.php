...
class BankController extends Controller
{
    public function index()
    {
        ...
        $tariktunais5 = TarikTunai::paginate(5)->sortByDesc('created_at');;

        $tariktunais = TarikTunai::all();
    ...
    public function tarik_tunai_store(Request $request)
    {
        ...
        $tarik_tunai = TarikTunai::find($request->tariktunai_id);
        $wallet->update([
    ...
    public function transactionindex()
    {
        ...
        $tariktunais = TarikTunai::where('status', 'confirmed')
        ->orWhere('status', 'rejected')
        ->orderBy('updated_at')
        ->get()->groupBy(function($item) {
          return Carbon::parse($item->updated_at)->format('Y-m-d');
    ...
    public function printtariktunais(string $date)
    {
        $date = Carbon::parse($date)->format('Y-m-d'); 
        $tariktunais = TarikTunai::whereIn('status', ['confirmed', 'rejected'])->whereDate('updated_at', $date)->get();
      

        return view('bank.printtariktunai', compact('tariktunais', 'date'));
    }
