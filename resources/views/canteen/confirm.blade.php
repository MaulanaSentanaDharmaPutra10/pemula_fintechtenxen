                        ...
                        @foreach ($transactions as $ts)
                            <tr>
                                ...
                                <td class="flex flex-col justify-center">
                                    ...
                                    <span class="text-xs text-green-500 mt-2">{{ Carbon\Carbon::parse($ts->updated_at)->diffForHumans() }}</span>
                                </td>
                                <td>{{ $ts->quantity }}</td>
                                <td>{{ $ts->user->name }}</td>
                                ...
                                <td>
                                    @if ($ts->status != "taken")
                                        <form action="{{ route('confirm.product', $ts->id) }}" method="post">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn bg-green-500">Confirm</button>
                                        </form>
                                    @else
                                        <span class="p-2 rounded-md bg-green-500">Confirmed</span>
                                    @endif
